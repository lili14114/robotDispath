...
class MyTools(object):
    ...
    def main_vice_TripNoValidation(self, main_record, vice_recordLst):
        '''
          验证行车记录主表司机和司机趟次是否与副表明细数据一一对应。
          :param main_record: 主表的一条数据
          :param vice_recordLst: 副表的全部记录，list列表
        ...
        main_record = main_record.split(' ')
        driver = main_record[4]
        driverTrip=float(main_record[5])
        pattern_tripNo = r'\d+.\d+'
        reg = re.compile(pattern_tripNo)
        tripNoLst = re.findall(reg, driver)
        #主表司机趟次合计
        sumTripNo=sum([float(i) for i in tripNoLst])
        #副表有效趟次合计。1、将无效的路单删除  2、删掉无效路单的列表数量即为有效路单趟次
        for i in range(len(vice_recordLst) - 1, -1, -1):
            if '有效' not in vice_recordLst[i]:
                vice_recordLst.pop(i)
        validRecordCount=float(len(vice_recordLst))
        if driverTrip!=sumTripNo or driverTrip!=validRecordCount:
            flag =False
    ...
    def random_time(self):
        ...
        for i in range(10):
            ...
            date = time.strftime("%H:%M:%S", date_touple)  # 将时间元组转成格式化字符串（1976-05-21）
        return date
    ...
    def loginplayLoad(self,username,password):
        '''
        调度系统登陆接口
        :param username:
        :param password:
        ...
        playload = {
            "username": self.base64(username),
            "password": self.base64(password),
            "jCaptchaCode": '111111'
        }
        return playload
...
if __name__ == '__main__':
    ...
    # flag=mytool.main_vice_TripNoValidation(main_record,vice_recordLst)
    print(mytool.listContain_text(Lst,text))
